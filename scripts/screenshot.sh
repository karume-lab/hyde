...
cat <<"USAGE"
 ...
 c Copy selected area to clipboard (no editor)
USAGE
}
...
# clipboard-only screenshot function (no editor)
take_screenshot_clipboard() {
    local mode=$1
    shift
    local extra_args=("$@")
    # execute grimblast with given args, copy only (no save)
    if "$LIB_DIR/hyde/grimblast" "${extra_args[@]}" copy "$mode"; then
        notify-send -a "HyDE Alert" "Screenshot copied to clipboard" "Screenshot copied successfully"
    else
        notify-send -a "HyDE Alert" "Screenshot Error" "Failed to take screenshot"
        return 1
    fi
...
case $1 in
    ...
    sc) #? 󱉶 Use 'tesseract' to scan image then add to clipboard
        check_package tesseract-data-eng a
        if ! GEOM=$(slurp); then
    ...
    ;;
    c) # copy selected area to clipboard without editor
        take_screenshot_clipboard "area"
    ;;
    *) # invalid option
        USAGE
    ;;
